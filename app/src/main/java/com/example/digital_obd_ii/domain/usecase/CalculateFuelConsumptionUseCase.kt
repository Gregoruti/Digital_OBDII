...
class CalculateFuelConsumptionUseCase @Inject constructor() {
     ...
     */
    fun litersPerHour(mafGs: Double, afr: Double = 14.7, fuelDensity: Double = 0.745, correctionFactor: Double = 1.0): Double {
        if (mafGs <= 0.0) return 0.0
        val baseLph = (mafGs * 3600) / (afr * fuelDensity * 1000)
        return baseLph * correctionFactor
    }
